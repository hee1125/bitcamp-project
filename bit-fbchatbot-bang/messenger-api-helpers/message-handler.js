...
addMessage('온도', (recipientId, messageText) => {
    ...
    sendAPI.sendTextMessage(recipientId, '현재온도: ' + awsIoT.objmap_dht.get('temp_value')+ '°C');
...
addMessage("미세먼지", (recipientId) => {
  ...
  sendAPI.sendTextMessage(recipientId, '현재미세먼지: ' + awsIoT.objmap_dht.get('dust_value')+ '[ug/m3]');
  api.callMessagesAPI(messageData);
})
